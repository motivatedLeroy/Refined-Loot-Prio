BINDING_HEADER_DKP = "Refined_Loot_Priority";
...
local function RLP_comms(self, event, prefix, message, channel, sender)
 if event == "CHAT_MSG_ADDON" then
  ...
  if tonumber(message) > RLP_version then 
   print(sender .. " hat eine neuere RLP version - ich nerve solange bis du updatest!")
  else return end
 ...
 elseif event == "PLAYER_ENTERING_WORLD" then -- send 
  C_ChatInfo.SendAddonMessage(AddonPrefix, RLP_version, "GUILD")
 end
end
